# Structured Directives (Pseudo Tool Calling)

This document defines the **Structured Directives** protocol for Vibe Tavern.
...
```json
{ "op": "set", "path": "/draft/character/name", "value": "Alice" }
```
...
- The helper `TavernKit::VibeTavern::Directives::Validator.normalize_patch_ops(...)`
  canonicalizes them to the v1 ops above (`set`, `delete`, `append`).
- Tolerance:
