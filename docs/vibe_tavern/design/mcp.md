...
`Tools::MCP::ToolExecutor` routes a local tool call via the mapping table, calls
`tools/call`, and wraps the MCP result into the standard VibeTavern tool result
envelope.
...
Code:
- `lib/tavern_kit/vibe_tavern/tools/mcp/tool_executor.rb`
...
- the app builds an MCP tool snapshot (`definitions` + `Tools::MCP::ToolExecutor`)
  - `Tools::MCP::ToolRegistryBuilder` is a convenience that can connect to servers and
    return `definitions` + `mapping` + started `clients`
- the app then assembles the final model-visible tool surface via
  `TavernKit::VibeTavern::ToolsBuilder`
- pass `surface.catalog` and `surface.executor` into `ToolCalling::ToolLoopRunner`
- ensure MCP clients are closed when done (they own subprocesses)
