  ...
  module VibeTavern
    module Directives
      module Validator
        ...
        # Validates a directives envelope and returns a normalized subset:
        #
        # - always requires {assistant_text:String, directives:Array}
        # - drops invalid directives (returns warnings instead of failing the whole envelope)
        # - allows the app to inject:
        #   - an allowlist of directive types (preferred)
        #   - alias mappings (for non-schema fallbacks)
        #   - per-directive payload validation (optional)
        ...
        # Helper for app-injected payload validators.
        #
        # @return [Hash, nil] error hash when invalid, otherwise nil.
        ...
        # Normalizes and validates patch ops.
        #
        # - Accepts common RFC6902-style op names (`add`, `replace`, `remove`) and
        #   canonicalizes them to V1 ops (`set`, `delete`).
        # - Keeps the format small and deterministic; callers can mutate payload
        #   with the normalized ops to improve downstream stability.
        #
        # @return [Hash] { ok: true, ops: Array<Hash> } on success
        #               { ok: false, code: String, details: Hash } on error
        def normalize_patch_ops(ops, allowed_path_prefixes: DEFAULT_ALLOWED_PATCH_PATH_PREFIXES)
          ...
          prefixes = Array(allowed_path_prefixes).map { |p| p.to_s }.reject(&:empty?)
