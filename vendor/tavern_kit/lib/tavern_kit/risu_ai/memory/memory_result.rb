  ...
  module RisuAI
    module Memory
      MemoryResult = Data.define(
        :blocks,           # Array<PromptBuilder::Block>
        :tokens_used,      # Integer
        :compression_type, # Symbol, nil
      ) do
        def initialize(blocks:, tokens_used: 0, compression_type: nil)
          blocks = Array(blocks)
          unless blocks.all? { |b| b.is_a?(TavernKit::PromptBuilder::Block) }
            raise ArgumentError, "blocks must be an Array<PromptBuilder::Block>"
          end
