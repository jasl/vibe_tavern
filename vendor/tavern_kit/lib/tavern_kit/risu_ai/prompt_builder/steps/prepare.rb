    ...
    module PromptBuilder
      module Steps
        ...
        class Prepare < TavernKit::PromptBuilder::Step
          ...
          def normalize_risuai_context!(ctx)
            context = ctx.context
            return if context.is_a?(TavernKit::RisuAI::Context)

            ctx.variables_store!
            if context.nil?
              ctx.context = TavernKit::RisuAI::Context.build({}, context: ctx, strict: ctx.strict?)
              return
            end

            unless context.is_a?(TavernKit::PromptBuilder::Context)
              raise ArgumentError, "risu_ai context must be a TavernKit::PromptBuilder::Context"
            end

            normalized = TavernKit::RisuAI::Context.normalize(context.to_h, context: ctx, strict: ctx.strict?)
            normalized.each do |key, value|
              context.set(key, value, allow_new: true)
            end
          end
