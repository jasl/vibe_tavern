...
module AgentCore
  module ContextManagement
    # Internal helper for keeping prompts within a token budget.
    #
    # Owns:
    # - ConversationState load/save semantics
    # - Sliding window drop strategy
    # - Optional auto-compaction (summarize dropped transcript)
    #
    # Does NOT own prompt assembly. Callers provide a build_prompt callable
    # that turns {summary, turns, memory_results} into a PromptBuilder::BuiltPrompt.
    class BudgetManager
      ...
      def initialize(
        ...
        @summarizer = Summarizer.new(provider: @provider, model: @model)
      end
      ...
      #
      # @param memory_results [Array] Initial memory results (may be dropped as needed)
      # @yieldparam summary [String, nil] Effective summary to inject (caller decides representation)
      # @yieldparam turns [Array<Array<Message>>] Transcript turns after state cursor (mutable)
      # @yieldparam memory_results [Array] Current memory results (mutable)
      # @return [PromptBuilder::BuiltPrompt]
      def build_prompt(memory_results:)
        raise ArgumentError, "build_prompt block is required" unless block_given?

        memory_results = Array(memory_results).dup
        ...
        transcript_read = read_messages_after_cursor(@chat_history, base_cursor)
        ...
        loop do
          ...
          if turns.any?
            ...
            if dropped_messages.size >= drop_flush_threshold
              if @auto_compact
                begin
                  compacted = summarize_messages(previous_summary: effective_summary, messages: dropped_messages)
                  effective_summary = compacted
