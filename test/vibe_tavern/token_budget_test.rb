...
class VibeTavernTokenBudgetTest < ActiveSupport::TestCase
  ...
  def with_default_token_budget(context_window_tokens:, reserved_response_tokens:)
    mod = TavernKit::VibeTavern::CapabilitiesRegistry
    original = mod.default_overrides
    mod.configure_default_overrides(
      context_window_tokens: context_window_tokens,
      reserved_response_tokens: reserved_response_tokens,
    )
    yield
  ensure
    mod.configure_default_overrides(**original)
  end

  test "RunnerConfig configures max_tokens step from capabilities token budget" do
    with_default_token_budget(context_window_tokens: 10, reserved_response_tokens: 2) do
      runner_config =
  ...
  test "RunnerConfig capabilities_overrides can override the default budget" do
    with_default_token_budget(context_window_tokens: 10, reserved_response_tokens: 2) do
      runner_config =
        TavernKit::VibeTavern::RunnerConfig.build(
          provider: "openai",
          ...
          capabilities_overrides: { context_window_tokens: 20, reserved_response_tokens: 3 },
        )

      entry = runner_config.pipeline[:max_tokens]
      assert_equal 20, entry.default_config.max_tokens
      assert_equal 3, entry.default_config.reserve_tokens
    end
  end

  test "PromptRunner rejects prompts that exceed the configured budget" do
    with_default_token_budget(context_window_tokens: 10, reserved_response_tokens: 0) do
      runner_config =
        TavernKit::VibeTavern::RunnerConfig.build(
          provider: "openai",
          model: "gpt-test",
